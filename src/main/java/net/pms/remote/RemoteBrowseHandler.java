	...
	private String mkBrowsePage(String id, HttpExchange t) throws IOException {
		...
		if (criteria != null) {
			...
			if (terms.length > 2) {
				...
				switch (terms[0]) {
				case "title":
					query.append("dc:title contains \"").append(terms[1]).append("\"");
					break;
				case "album":
					query.append("upnp:album contains \"").append(terms[1]).append("\"");
					break;
				case "artist":
...
//		}
        ...
        if (object instanceof MediaLibraryFolder
                && ((MediaLibraryFolder) object).getExpectedOutput() == MediaLibraryFolder.FILES) {
            SubSelect subSelector = ((MediaLibraryFolder) object).getSubSelector();
            if (subSelector != null && !res.contains(subSelector))
                res.add(subSelector);
        }
		...
		for (DLNAResource r : res) {
			...
			if (newId == null
					|| (r instanceof VirtualFolder && r.isDiscovered() && r.getChildren().size() == 0))
				continue;
...
//				}
		...
		vars.put("name", id.equals("0") ? configuration.getServerDisplayName() :
			(search != null) ? search :StringEscapeUtils.escapeHtml(res.get(0).getParent().getDisplayName()));
